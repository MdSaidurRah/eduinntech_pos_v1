...
async function bootstrap() {
    ...
    routes.isReady().then(() => {
        if (routes.currentRoute && routes.currentRoute.value && routes.currentRoute.value.meta && routes.currentRoute.value.meta.isFrontStore) {
            let warehouseSlug = routes.currentRoute.value.params.warehouse;

            if ((warehouseSlug === undefined || warehouseSlug === null || warehouseSlug === '') && window.config.frontStoreWarehouse && window.config.frontStoreWarehouse.slug) {
                warehouseSlug = window.config.frontStoreWarehouse.slug;
            }

            store.commit("front/updateWarehouseSlug", warehouseSlug);
